...
$plugin = array(
  ...
  'handler' => array(
    'class' => 'os_simpleview_default',
    'file'  => 'os_simpleview_default.inc',
    'title' => t('List of Posts'),
    'parent' => 'os_simpleview',),
);

class os_simpleview_default implements os_simpleview {
  function options_defaults(){ 
    //might have to collect defaults from all the boxes so we can construct...
    $options = array(
      'description' => '',
      'title' => '',
      'number_of_items' => 6,
      ...
      'content_type' => 'all',
      'sorts' => 'newest',
      'plugin_opts' => array(), //plugins can put extra options in here
    );
  ...
  public function os_simpleview_form_basics($options = NULL, $step = NULL){
    $form = array();
    ...
    $form['content_type'] = array(
      ...
      '#options' => array('all' => t('All')) + _os_simpleview_content_types(),
      '#required' => TRUE,
  ...
  public function os_simpleview_form_display_options($options = NULL, $step = NULL){
    ...
    $content_path = _os_simpleview_content_type_path($options['content_type']);
  ...
  function multiform_info() {    
    return array(      
    	'basics' => array(
      	'form id' => 'os_simpleview_form_basics', 
      	'title' => 'The Basics',   
      ),
      'display_options' => array(
      	'form id' => 'os_simpleview_form_display_options', 
  ...
  public function render($options = array()) {
    $view_settings = $this->view_settings($options); //pass along box options?
    if ($view = views_get_view($view_settings->view_name)) {
      if ($view->access($view_settings->display_id)) {
        ...
        //Append more link.  Not done in views because views can't control destination of link
        if ($options['more'] && count($view->result) >= intval($options['number_of_items'])) {
          $more = theme_more_link($options['more_link'], $options['more_text']);
          if (is_array($output)) {
            $output['content'] .= $more;
          } else {
            $output .= $more;
  ...
  function view_append($opts) {
    ...
    //content type filter
    $append += $this->_parse_options_content_type($opts['content_type']); 
 ...
 */
  ...
  function view_settings() {
    return (object)array('view_name' => 'os_simpleview_base', 'display_id' => 'default');
  } 
}