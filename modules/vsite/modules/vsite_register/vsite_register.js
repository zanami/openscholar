 ...
 *
 * Drupal.behaviors.vsite_register.toggle_user_forms swaps display of the new/existing user form elements.
 * This is determined by new user's display state.  So when both elements are showing, toggling will hide one and show
 * the other.
 *
  ...
  Drupal.behaviors.vsite_register = {
    toggle_user_forms: function() {
		...
		attach: function(ctx, settings) {
			...
			// Init display of new user or existing user
			if ($('#new_user_div').css('display') == $('#edit-existing-username').parent().css('display')) {
				Drupal.behaviors.vsite_register.toggle_user_forms();
			}

			// Attach click event to new user link.  only do this once, even if this behavior happens again.
